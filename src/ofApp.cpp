...
void ofApp::setup(){
  ...
  int camWidth = 320;
	...
	settings.videoPath = ofToDataPath("movie.mov", true);
...
void ofApp::draw(){
    ...
    shader.begin();
		shader.setUniformTexture("tex0", omxPlayer.getTextureReference(), 1);
    shader.setUniformTexture("tex1", omxPlayerNoise.getTextureReference(), 2);
    shader.setUniformTexture("tex2", image.getTextureReference(), 3);
    shader.setUniformTexture("imageMask", maskFbo.getTextureReference(), 4);
		omxPlayer.draw(0, 0, camWidth, camHeight);
		omxPlayerNoise.draw(0, 0, camWidth, camHeight);
